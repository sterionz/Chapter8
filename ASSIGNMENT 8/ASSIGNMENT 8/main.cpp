...
void option1()
{
    ...
    int cardsPerSuite = inputInteger("Enter a number of cards per suite: ", 0, 13);
...
void option3()
{
    ...
    double arrival_prob = 1.0 / 2.5; //  Customer/seconds , default 2.0/5 on average, a customer every 2.5 seconds.
    unsigned int display_interval = 10;
    unsigned int sim_speed_ms = 50;

    string speedSet = "DEFAULT";
    srand(static_cast<unsigned int>(time(0))); // Seed once here

    enum simSpeed {INSTANT = 0, FASTER = 10, FAST = 25, SLOW = 50, SLOWEST = 100};
    ...
    do
    {
        ...
        cout << "\n\t3> Simulation of checkout lines at CostCo using multiple queues STL";
        cout << "\n\t" << string(100, char(205));
        cout << "\n\t" << "Simulation Current Settings:";
        cout << "\n\t" << "Customer Arrival Probability: " << arrival_prob  * 100 << "%";
        cout << "\n\t" << "Display interval: " << display_interval << " seconds";

        switch (sim_speed_ms)
        {
        case INSTANT: speedSet = "INSTANT"; break;

        case FASTER: speedSet = "FASTER"; break;

        case FAST: speedSet = "FAST"; break;

        case SLOW: speedSet = "SLOW"; break;

        case SLOWEST: speedSet = "SLOWEST"; break;

        default:
            speedSet = "NON STANDARD SIM SPEED: " + to_string( sim_speed_ms) + " ms";
        }
        cout << "\n\t" << "Simulation Speed: " << speedSet;

        cout << "\n\t" << string(100, char(205));
        cout << "\n\t\t1> Run simulation";
        cout << "\n\t\t2> Change settings for sim speed";
        cout << "\n\t\t3> Default settings";
        cout << "\n\t" << string(100, char(196));
        ...
        case 2:
        {
            settings(arrival_prob, display_interval, sim_speed_ms);
            break;
        }
     
        case 3:
        {
            arrival_prob = 1.0 / 2.5; //  Customer/seconds , default 1.0/ 2.5 on average, a customer every 2.5 seconds.
            display_interval = 10;
            sim_speed_ms = 50;

            cout << "\t\tSimulation settings reset to default!\n\t";
            break;
        }
